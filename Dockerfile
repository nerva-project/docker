...
RUN apt update && apt upgrade --yes && export DEBIAN_FRONTEND=noninteractive && \
    ...
    sudo git ca-certificates nano screen wget curl apache2 php php-curl php-bcmath && apt clean

RUN curl -sL https://deb.nodesource.com/setup_13.x -o nodesource_setup.sh && bash nodesource_setup.sh

RUN apt update && export DEBIAN_FRONTEND=noninteractive && apt install --yes nodejs
...
RUN export DEBIAN_FRONTEND=noninteractive && apt install --no-install-recommends --yes \
    git build-essential libzmq3-dev libunbound-dev libboost-all-dev \
...
RUN cd /src/nerva/builder && export THREAD_COUNT=6 && ./build && \
